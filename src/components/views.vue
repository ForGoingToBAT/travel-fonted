    ...
    <div class="container" style="padding-left: 0;padding-right: 0">
        ...
        <div class="content" style="margin-top: 20px;">
            <div class="view-image">
                <div class="col-md-8" style="position: relative;">
                    ...
                    <span class="images">共<span>{{ viewIntroduce.imagesnum }}</span>张图片</span>
                </div>
                <div class="col-md-4">
        ...
        <div class="views-comments" id="comment-dot">
            ...
            <div class="media" v-for="(userinfo, index) in userinfos" v-bind:key="index">
                <img :src="userinfo.userFont" class="mr-3" style="
            width: 40px;
            height: 40px;
            border-radius: 50%;
          ...
          "/>
                <div class="media-body">
                    ...
                    <span class="s-star s-common" :class="'s-star' + userinfo.stars"></span><br/>
                    <p class="comments-content">{{ userinfo.comments }}</p>
                    <!--照片墙-->
                    <div class="views-img">
                        <img :src="image.pic" v-for="(image, index) in userinfo.viewImages" v-bind:key="index" class="col-md-3"/>
                    </div>
                    <div class="info clearfix">
                        <a class="view-comment" title="添加评论" href="javascript:void(0)" @click="writeComment(1)">评论</a>
                        <span class="time">{{ userinfo.commentTime }}</span>
                    </div>
                    <hr style="margin-top: 5px;"/>
                    <div class="views-replys">
                        <div class="views-reply d-flex" v-for="(reply, index) in userinfo.replys" v-bind:key="index">
                            <div class="user-font">
                                <img :src="reply.replyUserFont" style="width: 15px; height: 15px;"/>
                            </div>
                            <div class="user-reply col-md-12">
                                <a href="#" class="username">{{ reply.replyName }}</a>&nbsp;&nbsp;:&nbsp;&nbsp;<span>{{ reply.replyContent }}</span>
                                <a href="javascript:void(0)" style="margin-left: 5px; color: #deb887;" @click="writeComment(2)">回复</a><br/>
                                <span class="time">{{ reply.replyTime }}</span><br/>
                            </div>
                        </div>
                    </div>
                    <!--评论的窗口-->
                    <div class="reply-window" style="margin-top: 10px; display: none;">
                        <textarea placeholder="" class="col-md-12" style="border: 1px solid #deb887;"/>
                        <button class="btn btn-primary btn-sm" style="float: right;">回复</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    export default {
        ...
        methods: {
            writeComment: function (message) {
                if (message === 1) {
                    $(event.target).parents(".media-body").children(".reply-window").children("textarea").attr("placeholder", "发表评论");
                } else {
                    let username = $(event.target).parent().children(".username").html();
                    $(event.target).parents(".media-body").
                    children(".reply-window").children("textarea").attr("placeholder", "回复" + username);
                }
                if (this.reply_window) {
                    $(event.target).parents(".media-body").children(".reply-window").css({
                        display: "block",
                    });
                    $(event.target).parents(".media-body").children(".reply-window").children("textarea").animate({
                        height: "62px",
                    });
                } else {
                    $(event.target).parents(".media-body").children(".reply-window").children("textarea").animate({
